...
import { PaymentEvent, ClassificationResult, StrategyDecision } from "./backend/types";
import { prisma } from "./backend/db/prismaClient";

// Mock the database call so the simulation runs instantly without Postgres
// @ts-ignore
prisma.auditLogEntry.aggregate = async () => ({ _sum: { interventionCost: 0 } });
...
async function runSimulation() {
    ...
    for (let i = 0; i < NUM_SIMULATIONS; i++) {
        ...
        rootCausesArray.push(rootCause);
    ...
    // ---------------------------------------------------------
    // We evaluate each event individually in the simulation so the circuit 
    // breaker doesn't trip and halt the simulation after 100 transactions.
    const decisions: StrategyDecision[] = [];
    for (let i = 0; i < NUM_SIMULATIONS; i++) {
        const [decision] = await decideBatch([events[i]], [classifications[i]]);
        decisions.push(decision);
    }
    
    for (let i = 0; i < NUM_SIMULATIONS; i++) {
        const decision = decisions[i];
        const amount = amountsArray[i];
        ...
        // "escalate_human" and "no_action" cost 0 and recover 0.
    }
